...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verificar se senha foi informada e tem tamanho mínimo
    if (!empty($password) && strlen($password) < 6) {
        $erros[] = 'A senha deve ter pelo menos 6 caracteres.';
    }

    // Verificar se usuário já existe (exceto para o próprio)
    foreach ($usuarios as $index => $usuario) {
        if ($index != $usuarioIndex && $usuario['username'] === $username) {
            $erros[] = 'Este nome de usuário já está cadastrado para outro usuário.';
            break;
        }
        if ($index != $usuarioIndex && $usuario['email'] === $email) {
    ...
    if ($usuarioAtual['id'] == $_SESSION['usuario_id'] && $ativo === false) {
        $erros[] = 'Você não pode desativar seu próprio usuário.';
    }

    // Verificar se está tentando rebaixar o próprio usuário de admin
    if ($usuarioAtual['id'] == $_SESSION['usuario_id'] &&
            ($usuarioAtual['nivel'] === 'admin' && $nivel !== 'admin')) {
        $erros[] = 'Você não pode rebaixar seu próprio nível de administrador.';
    }
    ...
    if (empty($erros)) {
        ...
        $usuarios[$usuarioIndex]['nivel'] = $nivel;
        $usuarios[$usuarioIndex]['ativo'] = $ativo;
        $usuarios[$usuarioIndex]['data_atualizacao'] = date('Y-m-d H:i:s');

        // Atualizar senha se foi informada
        if (!empty($password)) {
            $usuarios[$usuarioIndex]['password'] = password_hash($password, PASSWORD_DEFAULT);
        }
        ...
        if (salvarArquivoJSON('../data/usuarios.json', $usuarios)) {
            ...
            $usuarioAtual = $usuarios[$usuarioIndex];
        } else {
...
<head>
    ...
    <title>Editar Usuário - Sistema de Gestão</title>
...
<main class="main-container">
    <div class="page-header">
        <div>
            <h1><i class="fas fa-edit"></i> Editar Usuário</h1>
    ...
    <div class="info-card">
        ...
        <div class="info-grid">
            <div class="info-item">
                ...
                <span class="info-value"><?php echo $usuarioAtual['id']; ?></span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
                ...
                <span class="info-value"><?php echo isset($usuarioAtual['data_atualizacao']) ? formatarData($usuarioAtual['data_atualizacao']) : '---'; ?></span>
            </div>
        </div>
    </div>
        ...
        <form method="POST" action="" class="form-card">
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="nome" name="nome" value="<?php echo htmlspecialchars($usuarioAtual['nome']); ?>" required class="form-control" placeholder="Digite o nome completo">
                </div>

                <div class="form-group">
                    <label for="username"><i class="fas fa-id-badge"></i> Nome de Usuário <span class="required">*</span></label>
                    <input type="text" id="username" name="username" value="<?php echo htmlspecialchars($usuarioAtual['username']); ?>" required class="form-control" placeholder="Ex: admin, joao.silva">
                ...
                <div class="form-group">
                    ...
                    <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($usuarioAtual['email']); ?>" required class="form-control" placeholder="usuario@empresa.com.br">
                </div>
                ...
                <div class="form-group">
                    ...
                    <small class="form-text">A senha deve ter pelo menos 6 caracteres (opcional)</small>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="nivel" name="nivel" required class="form-control">
                        <option value="view" <?php echo ($usuarioAtual['nivel'] ?? 'user') === 'view' ? 'selected' : ''; ?>>Visualizador (Apenas visualização)</option>
                        <option value="user" <?php echo ($usuarioAtual['nivel'] ?? 'user') === 'user' ? 'selected' : ''; ?>>Usuário Comum</option>
                        <option value="admin" <?php echo ($usuarioAtual['nivel'] ?? 'user') === 'admin' ? 'selected' : ''; ?>>Administrador</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="toggle-switch">
                        <input type="checkbox" id="ativo" name="ativo" value="1" <?php echo (isset($usuarioAtual['ativo']) && $usuarioAtual['ativo'] === true) ? 'checked' : ''; ?>>
            ...
            <?php if ($usuarioAtual['id'] == $_SESSION['usuario_id']): ?>
                <div class="warning-card" style="margin-bottom: var(--spacing-lg);">
                    <div class="warning-header">
                        <i class="fas fa-info-circle"></i>
                        <h4>Informação</h4>
                    </div>
                    <p>Você está editando seu próprio usuário. Alterações em seu nível de acesso ou status podem afetar sua sessão atual.</p>
                </div>
            <?php endif; ?>

            <div class="form-actions">
                <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Atualizar Usuário</button>
