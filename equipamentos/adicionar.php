...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($equipamentos as $equipamento) {
        ...
        if ($tipo === 'notebook' && !empty($hostname) && isset($equipamento['hostname']) && $equipamento['hostname'] === $hostname) {
    ...
    if (empty($erros)) {
        // Criar objeto de especificações
        $especificacoes = [];
        $especsRequeridas = getEspecificacoesPorTipo($tipo);
        
        if ($especsRequeridas['ram'] && !empty($ram)) {
            $especificacoes['ram'] = $ram;
        }
        if ($especsRequeridas['processador'] && !empty($processador)) {
            $especificacoes['processador'] = $processador;
        }
        if ($especsRequeridas['hd'] && !empty($hd)) {
            $especificacoes['hd'] = $hd;
        }
        ...
        $novoEquipamento = [
            ...
            'serial' => $serial ?: null,
            'hostname' => in_array($tipo, ['notebook', 'desktop']) ? $hostname : null,
...
<main class="main-container">
    ...
    <div class="form-card-container">
        <form method="POST" action="" class="form-card" id="form-equipamento">
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label for="hostname">
                        ...
                        <span>Hostname</span>
                        <span id="hostname-required" class="required" style="display: none;">*</span>
                    </label>
                    <input type="text" id="hostname" name="hostname"
                           value="<?php echo htmlspecialchars($_POST['hostname'] ?? ''); ?>"
                           class="form-control"
            ...
            <!-- Seção de Especificações Técnicas (visível para Desktop, Notebook e Celular) -->
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="ram" name="ram"
                               value="<?php echo htmlspecialchars($_POST['ram'] ?? ''); ?>"
                               class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="processador" name="processador"
                               value="<?php echo htmlspecialchars($_POST['processador'] ?? ''); ?>"
                               class="form-control"
                               placeholder="Ex: Intel Core i5, Ryzen 7">
                        <small class="form-text">Opcional</small>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="hd">
                            ...
                            <span>Armazenamento (HD/SSD)</span>
                        </label>
                        <input type="text" id="hd" name="hd"
                               value="<?php echo htmlspecialchars($_POST['hd'] ?? ''); ?>"
                               class="form-control"
                               placeholder="Ex: 256GB SSD, 1TB HD">
                        <small class="form-text">Opcional</small>
                    </div>
                </div>
            </div>
                ...
                <div class="status-options">
                    <label class="status-option">
                        <input type="radio" name="status" value="estoque" 
                               <?php echo (!isset($_POST['status']) || $_POST['status'] === 'estoque') ? 'checked' : ''; ?>
                    ...
                    <label class="status-option">
                        <input type="radio" name="status" value="alocado" 
                               <?php echo (isset($_POST['status']) && $_POST['status'] === 'alocado') ? 'checked' : ''; ?>
                    ...
                    <label class="status-option">
                        <input type="radio" name="status" value="emprestado" 
                               <?php echo (isset($_POST['status']) && $_POST['status'] === 'emprestado') ? 'checked' : ''; ?>
                               onchange="toggleColaboradorSelect(true)">
                    ...
                    <label class="status-option">
                        <input type="radio" name="status" value="manutencao" 
                               <?php echo (isset($_POST['status']) && $_POST['status'] === 'manutencao') ? 'checked' : ''; ?>
                               onchange="toggleColaboradorSelect(false)">
                    ...
                    <label class="status-option">
                        <input type="radio" name="status" value="fora_uso" 
                               <?php echo (isset($_POST['status']) && $_POST['status'] === 'fora_uso') ? 'checked' : ''; ?>
                               onchange="toggleColaboradorSelect(false)">
            ...
            <div class="form-group" id="colaborador-select" 
                 style="display: <?php echo (isset($_POST['status']) && in_array($_POST['status'], ['alocado', 'emprestado'])) ? 'block' : 'none'; ?>;">
...
<script>
    // Função para mostrar/ocultar select de colaborador
    ...
    // Função para controlar obrigatoriedade do hostname e mostrar especificações
    function toggleEspecificacoes() {
        ...
        const tiposComEspecificacoes = ['desktop', 'notebook', 'celular'];
        if (tiposComEspecificacoes.includes(tipo)) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Inicializar especificações e hostname
        toggleEspecificacoes();
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                // Validar hostname para notebooks e desktops
